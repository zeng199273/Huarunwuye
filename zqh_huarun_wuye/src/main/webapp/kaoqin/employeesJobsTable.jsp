...
<body>
	<fieldset class="layui-elem-field layui-field-title" style="margin-top: 20px;">
	    <legend style="text-align: center">员工职务详情界面</legend>
	</fieldset>
...
<script>
	layui.use(['table','form'], function(){
	  ...
	  table.render({
	    ...
	    ,url:'<%=contextPath %>/findEmployeesJobs'  //数据接口
	    ...
	    ,cols: [[   //设置表头。值是一个二维数组。方法渲染方式必填
	      ...
	      ,{field:'entryDate', title:'入职时间', width:140}
	      ,{field:'phone', title:'联系电话', width:140}
	      ,{field:'staffId', title:'员工号', width:80}
	      ,{field:'jobs', title:'职务', width:100}
	      ,{field:'basePay', title:'基本工资', width:100}
	      ,{field:'subsidy', title:'补贴', width:80}
	      ,{field:'grossPay', title:'实发工资', width:100}
	    ]]
	    ,page: true //分页
	  });
	//监听查询点击事件
      form.on('submit(queryForm)', function (data) {
              table.render({
                  elem: '#test'
                  , url: '<%=contextPath%>/findEmployeesJobs?name='+data.field.name //后台数据接口
                  , page: true //开启分页
                  ,cols: [[   //设置表头。值是一个二维数组。方法渲染方式必填
           	      {type: 'checkbox', fixed: 'left'}
           	      ,{field:'id', title:'编号', width:80, 
